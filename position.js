...
// 坐标转换函数
const position2Coordinate = function (position, size) {
  return [Math.floor(position / size), position % size];
};

const coordinate2Position = function (x, y, size) {
  return x * size + y;
};

// a和b是否在一条直线上，且距离小于maxDistance
const isLine = function (a, b, size) {
...
const isAllInLine = function (p, arr, size) {
...
const hasInLine = function (p, arr, size) {
  for (let i = 0; i < arr.length; i++) {
    if (isLine(p, arr[i], size)) {
      return true;
    }
